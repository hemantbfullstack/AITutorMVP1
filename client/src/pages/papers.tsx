...
export default function Papers() {
  ...
  // Fetch user's papers
  const { data: papers = [], isLoading: papersLoading } = useQuery<GeneratedPaper[]>({
    queryKey: ["/api/papers"],
    retry: false,
  });
  ...
  const generateMutation = useMutation({
    ...
    onSuccess: async (result: any) => {
      ...
      toast({
        ...
        description: "Your IB Mathematics paper has been generated successfully!",
      });
    },
    onError: (error) => {
  ...
  const downloadMutation = useMutation({
    mutationFn: async ({ paperId, type }: { paperId: string; type: "paper" | "markscheme" }) => {
    ...
    onSuccess: (blob, variables) => {
      ...
      toast({
        ...
        description: `${variables.type === "paper" ? "Paper" : "Markscheme"} PDF download started.`,
      });
    },
    onError: (error) => {
      toast({
      ...
      <div className="pt-20 pb-8 px-4">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Generate authentic IB-style mathematics papers with automated markschemes
          </p>
        </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => setFormData(prev => ({ ...prev, subject: e.target.value as "AA" | "AI" }))}
                        data-testid="select-subject"
                      >
                        <option value="AA">Analysis & Approaches (AA)</option>
                        <option value="AI">Applications & Interpretation (AI)</option>
                      </select>
                    </div>
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => setFormData(prev => ({ ...prev, level: e.target.value as "HL" | "SL" }))}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => setFormData(prev => ({ ...prev, paperType: e.target.value as "P1" | "P2" }))}
                        data-testid="select-paper-type"
                      >
                        <option value="P1">Paper 1 (No calculator)</option>
                        <option value="P2">Paper 2 (Calculator allowed)</option>
                      </select>
                    </div>
                    <div>
                ...
                <Card>
                  <CardHeader>
                    <div className="flex justify-between items-start">
                      <div>
                        <CardTitle>
                          {selectedPaper.subject} {selectedPaper.level} Paper {selectedPaper.paperType.slice(1)}
                        </CardTitle>
                        <CardDescription>
                          {selectedPaper.totalMarks} marks • {selectedPaper.questionsJson.length} questions
                          {selectedPaper.paperType === "P1" ? " • No calculator" : " • Calculator allowed"}
                        </CardDescription>
                      </div>
                  ...
                  <CardContent>
                    <div className="flex gap-2 mb-4">
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => downloadMutation.mutate({ paperId: selectedPaper.id, type: "paper" })}
                      ...
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => downloadMutation.mutate({ paperId: selectedPaper.id, type: "markscheme" })}
                        disabled={downloadMutation.isPending}
                      ...
                      {selectedPaper.questionsJson.map((question, index) => (
                        <div key={question.qId} className="border-b pb-4">
                          <div className="flex justify-between items-start mb-2">
                            <h4 className="font-semibold text-lg">{index + 1}.</h4>
                            <span className="text-sm text-gray-600 font-mono">[{question.marks}]</span>
                          </div>
                          ...
                          {showMarkscheme && (
                            <div className="mt-3 p-3 bg-gray-50 rounded-md">
                              <h5 className="font-semibold text-sm text-gray-700 mb-2">Markscheme:</h5>
                              {selectedPaper.markschemeJson
                                .find(ms => ms.qId === question.qId)
                                ?.steps.map((step, stepIndex) => (
                                <div key={stepIndex} className="flex justify-between items-start text-sm">
                                  <span className="flex-1">
                                    <strong>{step.label}:</strong> {renderMathContent(step.text)}
                                  </span>
                                  <span className="font-mono text-gray-600 ml-2">[{step.marks}]</span>
                                </div>
                              ))}
                            </div>
                          )}
              ...
              <CardContent>
                {papersLoading ? (
                  ...
                  <div className="text-center py-8 text-gray-500">
                    No papers generated yet. Create your first paper using the Generate tab.
                  </div>
                ) : (
                        ...
                        <div className="flex justify-between items-start">
                          <div>
                            <h3 className="font-semibold">
                              {paper.subject} {paper.level} Paper {paper.paperType.slice(1)}
                            </h3>
                            <p className="text-sm text-gray-600">
                              {paper.totalMarks} marks • {paper.questionsJson.length} questions
                            </p>
                          ...
                          <div className="flex gap-2">
                            ...
                            <Button
                              variant="outline"
                              size="sm"
                              onClick={() => downloadMutation.mutate({ paperId: paper.id, type: "paper" })}
                            ...
                            <Button
                              variant="outline"
                              size="sm"
                              onClick={() => downloadMutation.mutate({ paperId: paper.id, type: "markscheme" })}
