...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/auth/user", isAuthenticated, async (req: any, res) => {
    try {
      // Check if we're running in local development mode (forced by NODE_ENV=development)
      const isReplitEnvironment =
        !!(process.env.REPLIT_DOMAINS && process.env.REPL_ID) &&
        process.env.NODE_ENV !== "development";
      ...
      if (
        req.user &&
        req.user.claims &&
        req.user.claims.sub === "replit-dev-user"
      ) {
  ...
  app.post("/api/tutor/selftest", isAuthenticated, async (req: any, res) => {
    try {
      ...
      const testMessages = [{ role: "user" as const, content: "Say 'ready'" }];
  ...
  app.post(
    ...
    async (req: any, res) => {
      ...
      try {
        const userId = getUserId(req);
        const { message, ibSubject, ibLevel, sessionId } =
          tutorMessageSchema.parse(req.body);
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        const { subject, level, paperType, numQuestions, topics, calcAllowed } =
          generatePaperSchema.parse(req.body);
