...
import { users, tutorSessions, messages, generatedPapers } from '../../shared/schema';
...
router.get('/users', async (req, res) => {
  try {
    ...
    const allUsers = await db
      ...
      .where(whereClause)
      .orderBy(
        sortBy === 'firstName' ? (sortOrder === 'asc' ? asc(users.firstName) : desc(users.firstName)) :
        sortBy === 'lastName' ? (sortOrder === 'asc' ? asc(users.lastName) : desc(users.lastName)) :
        sortBy === 'email' ? (sortOrder === 'asc' ? asc(users.email) : desc(users.email)) :
        sortBy === 'role' ? (sortOrder === 'asc' ? asc(users.role) : desc(users.role)) :
        sortBy === 'planId' ? (sortOrder === 'asc' ? asc(users.planId) : desc(users.planId)) :
        sortBy === 'usageCount' ? (sortOrder === 'asc' ? asc(users.usageCount) : desc(users.usageCount)) :
    ...
    if (userIds.length > 0) {
      ...
      const statsPromises = userIds.map(async (userId) => {
        const [hasSessions, hasMessages, hasPapers] = await Promise.all([
          db.select({ exists: sql<boolean>`EXISTS(SELECT 1 FROM ${tutorSessions} WHERE ${tutorSessions.userId} = ${userId})` }),
          db.select({ exists: sql<boolean>`EXISTS(SELECT 1 FROM ${messages} WHERE ${messages.userId} = ${userId})` }),
          db.select({ exists: sql<boolean>`EXISTS(SELECT 1 FROM ${generatedPapers} WHERE ${generatedPapers.userId} = ${userId})` })
        ]);
...
router.get('/users/stats', async (req, res) => {
  try {
    const [totalUsers, activeUsers, newUsersThisMonth, premiumUsers] = await Promise.all([
      ...
      db.select({ count: count() }).from(users).where(sql`${users.usageCount} > 0`),
      db.select({ count: count() }).from(users).where(sql`${users.createdAt} >= NOW() - INTERVAL '1 month'`),
      db.select({ count: count() }).from(users).where(sql`${users.planId} IN ('monthly', 'annual')`)
    ]);
...
router.get('/stats', async (req, res) => {
  try {
    const [userCount, sessionCount, messageCount, paperCount] = await Promise.all([
      db.select({ count: count() }).from(users),
